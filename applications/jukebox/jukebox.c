...
static void jukebox_send_signal(uint32_t frequency, string_t signal, string_t protocol) {
	...
	string_init_printf(flipper_format_string, "File_name: %s",
...
static void jukebox_render_callback(Canvas *canvas, void *ctx) {
	JukeboxAppState *state = (JukeboxAppState*) acquire_mutex((ValueMutex*) ctx,
			25);
	canvas_clear(canvas);
	char strings[5][20];
	string_t signal;
...
//    canvas_draw_circle(canvas, 100, 26, 25);
	...
	if (strlen(string_get_cstr(signal)) > 12) {
		...
		flipper_format_read_uint32(fff_data_file, "Frequency",
				(uint32_t*) &frequency_str, 1);
...
int32_t jukebox_app(void *p) {
	...
	Storage *storage = furi_record_open("storage");
	FlipperFormat *fff_data_file = flipper_format_file_alloc(storage);
	if (!flipper_format_file_open_existing(fff_data_file,
			string_get_cstr(file_name))) {
	...
	flipper_format_free(fff_data_file);
	furi_record_close("storage");
	FURI_LOG_I(TAG, "%s %s %s %s %s ", string_get_cstr(up_file),
			string_get_cstr(down_file), string_get_cstr(left_file),
			string_get_cstr(right_file), string_get_cstr(ok_file));

	JukeboxAppState _state = { { false, false, false, false, false }, 0, 0, 0,
			0, 0 };
